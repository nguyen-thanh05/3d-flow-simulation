...
def incremental_material_balance(S1_old, S1_new, P_old, P_new, V_pore, pvt_table, prod_grid, inj_grid, dt):
    c_f = pvt_table.get_compressibility_f(P_old)
    c_w = pvt_table.get_compressibility_w(P_old)
    c_o = pvt_table.get_compressibility_o(P_old)
    B_w = pvt_table.get_B_w(P_old)
    B_o = pvt_table.get_B_o(P_old)
    lhs = V_pore * (c_f + S1_old * c_w + (1 - S1_old) * c_o) * (P_new - P_old)
    lhs = lhs.sum()
